...
pub mod CounterContract{
    ...
    impl CounterImpl of ICounter<ContractState>{
        ...
        fn reset_counter(ref self: ContractState){
            ...
            let dispacher = IERC20Dispatcher {contract_address: strk_token};

            let balance = dispacher.balance_of(caller);
            assert!(balance >= payment_amount,"User doesn't have enough balance");

            let allowance = dispacher.allowance(caller,contract);
            assert!(allowance >= payment_amount,"Contract is not allowed to spend enough STRK");

            let owner = self.ownable.owner();
            let success = dispacher.transfer_from(caller,owner,payment_amount);
